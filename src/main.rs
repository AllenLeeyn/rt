use rand::Rng;
use rt_2::core::*;
use rt_2::material::*;
use rt_2::objects::*;
use rt_2::{random_double, random_uszie};
use rt_2::scene::*;
use rt_2::pixels::*;
use rt_2::particle_system::*;
use clap::Parser;

#[derive(Parser, Debug)]
#[command(author, version, about, long_about = None)]
struct Args {
    /// Scene number to render (1-4)
    #[arg(short = 's', long = "scene", default_value_t = 3)]
    scene: u32,

    /// Output filename
    #[arg(short = 'o', long = "output", default_value = "output.ppm")]
    output: String,

    /// Resolution width and height
    #[arg(short = 'r', long = "resolution", value_names = &["WIDTH", "HEIGHT"])]
    resolution: Option<Vec<u32>>,
    
    /// Samples per pixel
    #[arg(short = 'q', long = "quality", default_value_t = 32)]
    samples: u32,

    /// depth per pixel
    #[arg(short = 'd', long = "depth", default_value_t = 10)]
    depth: u32,
}

fn main() -> std::io::Result<()> {
    let args = Args::parse();

    let mut scene = Scene::new();

    // Select scene
    match args.scene {
        1 => scene_one(&mut scene),
        2 => scene_two(&mut scene),
        3 => scene_three(&mut scene),
        4 => scene_four(&mut scene),
        5 => scene_five(&mut scene),
        6 => scene_six(&mut scene),
        _ => {
            eprintln!("Unknown scene {}, defaulting to scene_three", args.scene);
            scene_three(&mut scene);
        }
    }

    // Set resolution if provided and exactly 2 values passed
    if let Some(res) = &args.resolution {
        if res.len() == 2 {
            scene.camera_mut().set_resolution((res[0], res[1]));
        } else {
            eprintln!("Resolution requires exactly two values: width and height");
        }
    }

    scene.set_sample_size(args.samples);
    scene.set_max_depth(args.depth);
    scene.render(&args.output)?;

    Ok(())
}

fn scene_one(scene: &mut Scene) {
    scene.camera_mut().set(
        Point3::splat(4.0),
        Vec3::ZERO,
        Vec3::Y,
        60.0,
        1.0,
        (400, 300));

    scene.add_object(Sphere::new(
        Point3::ZERO,
        2.0,
        Material{
            texture: Texture::SolidColor(Color::PASTEL_LIME),
            diffuse: 1.0,
            reflectivity: 0.0,
            transparency: 0.0,
            ior: 0.0,
            emission: None,
            specular: 0.0,
            shininess: 0.0,
            is_volume: false,
            density: 0.0,
        },
    ));
    
    scene.add_light(Light::new_directional(
        Point3::new(0.0, -4.0, -2.0),
        Color::WHITE,
        1.0,
    ));
}

fn scene_two(scene: &mut Scene) {
    scene.camera_mut().set(
        Point3::splat(3.0),
        Vec3::ZERO,
        Vec3::Y,
        60.0,
        1.0,
        (400, 300));

    scene.add_object(Plane::new(
        Point3::ZERO,
        Vec3::new(5.0, 0.01, 5.0),
        Material{
            texture: Texture::Checkerboard(
                Color::LIGHT_GRAY,
                Color::GRAY,
                5.0
            ),
            diffuse: 0.01,
            reflectivity: 0.0,
            transparency: 0.0,
            ior: 0.0,
            emission: None,
            specular: 0.0,
            shininess: 0.0,
            is_volume: false,
            density: 0.0,
        },
    ));
    
    let image = Image::load("assets/test.png").unwrap();
    scene.add_object(Cube::new(
        Point3::new(0.0, 0.5, 0.0),
        1.0,
        Material{
            texture: Texture::Image(image),
            diffuse: 0.01,
            reflectivity: 0.0,
            transparency: 0.0,
            ior: 0.0,
            emission: None,
            specular: 0.0,
            shininess: 0.0,
            is_volume: false,
            density: 0.0,
        },
    ));
    
    scene.add_light(Light::new_directional(
        Point3::new(1.0, -4.0, -4.0),
        Color::WHITE,
        1.0,
    ));
}

fn scene_three(scene: &mut Scene) {
    scene.camera_mut().set(
        Point3::new(1.8,0.5, 4.0),
        Vec3::new(0.0, 1.5, -4.0),
        Vec3::new(-0.3, 1.0, 1.0),
        60.0,
        1.0,
        (400, 300));
    
    scene.set_background(Texture::SolidColor(Color::PASTEL_BLUE));
    
    scene.add_object(Cube::new(
        Point3::new(-7.0, 5.0, 0.0),
        10.0,
        Material{
            texture: Texture::SolidColor(Color::RED),
            diffuse: 1.0,
            reflectivity: 0.0,
            transparency: 0.0,
            ior: 0.0,
            emission: None,
            specular: 0.0,
            shininess: 0.0,
            is_volume: false,
            density: 0.0,
        },
    ));

    scene.add_object(Cube::new(
        Point3::new(7.0, 5.0, 0.0),
        10.0,
        Material{
            texture: Texture::SolidColor(Color::BLUE),
            diffuse: 1.0,
            reflectivity: 0.0,
            transparency: 0.0,
            ior: 0.0,
            emission: None,
            specular: 0.0,
            shininess: 0.0,
            is_volume: false,
            density: 0.0,
        },
    ));

    scene.add_object(Cube::new(
        Point3::new(0.0, 5.0, -7.0),
        10.0,
        Material{
            texture: Texture::SolidColor(Color::GREEN),
            diffuse: 0.0,
            reflectivity: 0.0,
            transparency: 0.4,
            ior: 0.0,
            emission: None,
            specular: 0.0,
            shininess: 0.0,
            is_volume: false,
            density: 0.0,
        },
    ));

    scene.add_object(Cube::new(
        Point3::new(0.0, 5.0, 10.0),
        10.0,
        Material{
            texture: Texture::SolidColor(Color::WHITE),
            diffuse: 1.0,
            reflectivity: 0.0,
            transparency: 0.0,
            ior: 0.0,
            emission: None,
            specular: 0.0,
            shininess: 0.0,
            is_volume: false,
            density: 0.0,
        },
    ));

    scene.add_object(Cube::new(
        Point3::new(0.0, 8.5, 0.0),
        10.0,
        Material{
            texture: Texture::SolidColor(Color::WHITE),
            diffuse: 1.0,
            reflectivity: 0.0,
            transparency: 0.0,
            ior: 0.0,
            emission: None,
            specular: 0.0,
            shininess: 0.0,
            is_volume: false,
            density: 0.0,
        },
    ));

    scene.add_object(Cube::new(
        Point3::new(0.0, 3.9, 0.0),
        1.0,
        Material{
            texture: Texture::SolidColor(Color::WHITE),
            diffuse: 0.0,
            reflectivity: 0.0,
            transparency: 0.0,
            ior: 0.0,
            emission: Some(Color::WHITE * 4.0),
            specular: 0.0,
            shininess: 0.0,
            is_volume: false,
            density: 0.0,
        },
    ));

/* 
    scene.add_light(Light::new_point(
        Point3::new(0.0, 3.2, 0.0),
        Color::WHITE,
        0.5,
        2,
        0.5,
        1.0,
    ));
 */
    scene.add_object(Cylinder::new(
        Point3::new(1.5, 0.0, 0.5),
        0.15,
        4.0,
        Material{
            texture: Texture::SolidColor(Color::WHITE),
            diffuse: 0.1,
            reflectivity: 2.0,
            transparency: 0.0,
            ior: 0.0,
            emission: None,
            specular: 0.0,
            shininess: 0.0,
            is_volume: false,
            density: 0.0,
        },
    ));

    scene.add_object(Cube::new(
        Point3::new(-1.0, 0.4, -1.1),
        0.8,
        Material{
            texture: Texture::SolidColor(Color::ORANGE),
            diffuse: 0.8,
            reflectivity: 0.0,
            transparency: 0.0,
            ior: 0.0,
            emission: None,
            specular: 0.1,
            shininess: 16.0,
            is_volume: false,
            density: 0.0,
        },
    ));

    scene.add_object(Cube::new(
        Point3::new(-1.0, 0.55, -0.85),
        0.6,
        Material{
            texture: Texture::SolidColor(Color::ORANGE),
            diffuse: 0.0,
            reflectivity: 0.0,
            transparency: 0.0,
            ior: 0.0,
            emission: Some(Color::ORANGE * 40.0),
            specular: 0.1,
            shininess: 16.0,
            is_volume: false,
            density: 0.0,
        },
    ));

    scene.add_object(Sphere::new(
        Point3::new(-0.4, 0.5, 1.0),
        0.5,
        Material{
            texture: Texture::SolidColor(Color::GREEN),
            diffuse: 0.0,
            reflectivity: 0.0,
            transparency: 0.92,
            ior: 1.49,
            emission: None,
            specular: 0.0,
            shininess: 0.0,
            is_volume: false,
            density: 0.0,
        },
    ));

    scene.add_object(Sphere::new(
        Point3::new(-0.4, 0.5, 1.0),
        -0.45,
        Material{
            texture: Texture::SolidColor(Color::GREEN),
            diffuse: 0.0,
            reflectivity: 0.0,
            transparency: 0.92,
            ior: 1.49,
            emission: None,
            specular: 0.0,
            shininess: 0.0,
            is_volume: false,
            density: 0.0,
        },
    ));

    scene.add_object(Plane::new(
        Point3::ZERO,
        Vec3::new(20.0, 0.0, 20.0),
        Material{
            texture: Texture::Checkerboard(Color::GRAY, Color::PASTEL_GRAY, 20.0),
            diffuse: 0.3,
            reflectivity: 0.3,
            transparency: 0.0,
            ior: 0.0,
            emission: None,
            specular: 0.1,
            shininess: 0.1,
            is_volume: false,
            density: 0.0,
        },
    ));
}

fn scene_four(scene: &mut Scene) {
    scene.camera_mut().set(
        Point3::new(-2.0, 2.0, -4.0),
        Vec3::new(0.0, 2.0, 0.0),
        Vec3::Y,
        60.0,
        1.0,
        (400, 300));
    
    scene.add_object(Cube::new(
        Point3::new(-1.5, 0.5, 0.5),
        1.0,
        Material{
            texture: Texture::Gradient(Color::DARK_RED, Color::RED, 1.571),
            diffuse: 0.1,
            reflectivity: 0.0,
            transparency: 0.0,
            ior: 0.0,
            emission: None,
            specular: 0.0,
            shininess: 0.0,
            is_volume: false,
            density: 0.0,
        },
    ));

    scene.add_object(Cylinder::new(
        Point3::new(2.0, 0.0, -1.0),
        0.25,
        2.0,
        Material{
            texture: Texture::Checkerboard(Color::BLUE, Color::YELLOW, 1.0),
            diffuse: 0.0,
            reflectivity: 0.0,
            transparency: 0.0,
            ior: 0.0,
            emission: None,
            specular: 0.0,
            shininess: 0.0,
            is_volume: false,
            density: 0.0,
        },
    ));

    scene.add_object(Sphere::new(
        Point3::new(-0.5, 0.5, 0.0),
        0.5,
        Material{
            texture: Texture::SolidColor(Color::GREEN),
            diffuse: 0.0,
            reflectivity: 0.0,
            transparency: 0.0,
            ior: 0.0,
            emission: None,
            specular: 0.0,
            shininess: 0.0,
            is_volume: false,
            density: 0.0,
        },
    ));

    scene.add_object(Plane::new(
        Point3::ZERO,
        Vec3::new(20.0, 0.0, 20.0),
        Material{
            texture: Texture::Checkerboard(Color::GRAY, Color::PASTEL_GRAY, 20.0),
            diffuse: 0.0,
            reflectivity: 0.0,
            transparency: 0.0,
            ior: 0.0,
            emission: None,
            specular: 0.0,
            shininess: 0.0,
            is_volume: false,
            density: 0.0,
        },
    ));
    scene.add_light(Light::new_directional(
        Point3::new(-1.5, -6.0, 3.0),
        Color::WHITE,
        0.05,
    ));

    scene.add_light(Light::new_directional(
        Point3::new(-3.0, -8.0, 3.0),
        Color::WHITE,
        0.4,
    ));
}

fn scene_five(scene: &mut Scene) {
    scene.camera_mut().set(
        Point3::new(0.0, 0.5, 1.0),
        Vec3::new(0.0, 0.5, 0.0),
        Vec3::Y,
        60.0,
        1.0,
        (400, 300));

    scene.set_background(
        Texture::Gradient(Color::PASTEL_BLUE,
            Color::WHITE, 1.571));

    scene.add_object(Plane::new(
        Point3::ZERO,
        Vec3::new(20.0, 0.0, 20.0),
        Material{
            texture: Texture::SolidColor(Color::YELLOW),
            diffuse: 0.5,
            reflectivity: 0.0,
            transparency: 0.0,
            ior: 0.0,
            emission: None,
            specular: 0.0,
            shininess: 0.0,
            is_volume: false,
            density: 0.0,
        },
    ));

    scene.add_object(Sphere::new(
        Point3::new(0.0, 0.5, -1.0),
        0.5,
        Material{
            texture: Texture::SolidColor(Color::new(0.7, 0.3, 0.3)),
            diffuse: 0.5,
            reflectivity: 0.0,
            transparency: 0.0,
            ior: 0.0,
            emission: None,
            specular: 0.0,
            shininess: 0.0,
            is_volume: false,
            density: 0.0,
        },
    ));

    scene.add_object(Sphere::new(
        Point3::new(-1.0, 0.5, -1.0),
        0.5,
        Material{
            texture: Texture::SolidColor(Color::new(0.8, 0.8, 0.8)),
            diffuse: 0.0,
            reflectivity: 1.0,
            transparency: 0.0,
            ior: 0.0,
            emission: None,
            specular: 0.0,
            shininess: 0.0,
            is_volume: false,
            density: 0.0,
        },
    ));

    scene.add_object(Sphere::new(
        Point3::new(1.0, 0.5, -1.0),
        0.5,
        Material{
            texture: Texture::SolidColor(Color::new(0.8, 0.7, 0.3)),
            diffuse: 0.0,
            reflectivity: 0.0,
            transparency: 1.0,
            ior: 1.6,
            emission: None,
            specular: 0.0,
            shininess: 0.0,
            is_volume: false,
            density: 0.0,
        },
    ));

    scene.add_object(Sphere::new(
        Point3::new(1.0, 0.5, -1.0),
        -0.45,
        Material{
            texture: Texture::SolidColor(Color::DARK_PURPLE),
            diffuse: 0.0,
            reflectivity: 0.,
            transparency: 0.7,
            ior: 1.5,
            emission: None,
            specular: 0.0,
            shininess: 0.0,
            is_volume: false,
            density: 0.0,
        },
    ));

    scene.add_object(Sphere::new(
        Point3::new(1.0, 0.5, -1.0),
        0.3,
        Material{
            texture: Texture::SolidColor(Color::DARK_ORANGE),
            diffuse: 0.0,
            reflectivity: 0.0,
            transparency: 0.6,
            ior: 1.5,
            emission: None,
            specular: 0.0,
            shininess: 0.0,
            is_volume: false,
            density: 0.0,
        },
    ));

    scene.add_light(Light::new_point(
        Point3::new(0.0, 3.0, 1.0),
        Color::WHITE,
        1.0,
        4,
        0.8,
        1.0
    ));
}

fn scene_six(scene: &mut Scene) {
    scene.camera_mut().set(
        Point3::new(4.0, 2.0, 4.0),
        Vec3::new(0.0, 2.0, 0.0),
        Vec3::Y,
        60.0,
        1.0,
        (400, 300));

    scene.set_background(
        Texture::Gradient(Color::DARK_BLUE * 0.1,
            Color::DARK_CYAN * 0.1, 1.571));

    scene.add_object(Sphere::new(
        Point3::new(-0.0, 6.0, -6.0),
        0.75,
        Material { 
            texture: Texture::SolidColor(Color::ORANGE),
            diffuse: 0.0, 
            reflectivity: 0.0,
            transparency: 0.0,
            ior: 0.0,
            emission: Some(Color::ORANGE * 20.0),
            specular: 0.0,
            shininess: 0.0,
            is_volume: false,
            density: 0.0,
        })
    );

    add_tree(scene, 0.0, 0.0, 0.0);
    add_tree(scene, -0.6, 0.4, -2.0);
    add_tree(scene, -0.5, -0.0, -3.0);
    add_tree(scene, -2.0, -0.2, 1.0);
    add_tree(scene, -3.0, 0.2, -1.0);
    add_tree(scene, 3.0, -0.25, -1.0);
    add_tree(scene, 1.5, 0.25, -2.5);
    add_tree(scene, -3.0, -0.1, 2.0);

    // Setup ParticleSys to create random spheres within a box
    let grounds = ParticleSys::new(
        Point3::new(-4.0, -0.4, -4.0), // min corner
        Point3::new(4.0, 0.1, 4.0),  // max corner
        64,                          // number of particles
        |i: usize, pos: Point3| {
            let grid_cols = 8;
            let col = i % grid_cols;
            let row = i / grid_cols;
            
            let spacing = 1.0;
            let x = -4.0 + col as f32 * spacing;
            let z = -4.0 + row as f32 * spacing;

            let size = 0.95;
            
            let green_shade = Color::new(
                0.1 + random_double() * 0.15,   // R
                0.5 + random_double() * 0.6,    // G
                0.1 + random_double() * 0.15,   // B
            );
            let material = Material {
                texture: Texture::SolidColor(green_shade),
                diffuse: 1.0,
                reflectivity: 0.0,
                transparency: 0.0,
                ior: 0.0,
                emission: None,
                specular: 0.0,
                shininess: 0.0,
                is_volume: false,
                density: 0.0,
            };

            Box::new(Cube::new(
                Point3::new(x , pos.y(), z),
                size,
                material
            )) as Box<dyn Hittable>
        },
        0.15
    );

    // Generate and add particles to the scene
    for ground in grounds.generate() {
        let pos = ground.as_ref().position();
        let mut flower_count = random_uszie().min(5);
        
        flower_count = if flower_count < 4 { 0 } else { flower_count - 2 };

        for _ in 0..flower_count {
            add_flower(
                scene,
                pos.x() - 0.4 + random_double() * 0.8,
                pos.y() + 0.5,
                pos.z() - 0.4 + random_double() * 0.8,
                0.03 + random_double() * 0.03,
            );
        }

        scene.add_boxed_object(ground);
    }

    let stars = ParticleSys::new(
        Point3::new(-30.0, 10.0, -30.0), // min corner
        Point3::new(-5.0, 15.0, -5.0),  // max corner
        20,                          // number of particles
        |_i: usize, pos: Point3| {
            let material = Material {
                texture: Texture::SolidColor(Color::WHITE),
                diffuse: 0.0,
                reflectivity: 0.0,
                transparency: 0.0,
                ior: 0.0,
                emission: Some(Color::WHITE * 2.0),
                specular: 0.0,
                shininess: 0.0,
                is_volume: false,
                density: 0.0,
            };

            Box::new(Sphere::new(
                pos,
                0.05,
                material
            )) as Box<dyn Hittable>
        },
        3.0
    );

    // Generate and add particles to the scene
    for star in stars.generate() {
        scene.add_boxed_object(star);
    }

    let clouds = ParticleSys::new(
        Point3::new(-3.0, 4.5, -6.0), // min corner of bounding box
        Point3::new(2.0, 5.0, -3.0),   // max corner
        15,                           // number of cloud particles
        |_i: usize, pos: Point3| {
            let radius = 0.2 + random_double() * 0.4; // 0.5 to 0.8

            let boundary_sphere = Box::new(Sphere::new(pos, radius, Material {
                // Dummy material for boundary
                texture: Texture::SolidColor(Color::WHITE),
                diffuse: 0.0,
                reflectivity: 0.0,
                transparency: 0.0,
                ior: 0.0,
                emission: None,
                specular: 0.0,
                shininess: 0.0,
                is_volume: false,
                density: 0.0,
            }));

            let cloud_material = Material {
                texture: Texture::SolidColor(Color::new(0.9, 0.9, 1.0)), // soft cloudy white
                diffuse: 0.7,
                reflectivity: 0.0,
                transparency: 1.0,
                ior: 0.0,
                emission: Some(Color::PASTEL_BLUE * 2.0),
                specular: 0.0,
                shininess: 0.0,
                is_volume: true,
                density: 0.4, // tweak for softness
            };

            let volume = Volumes::new(boundary_sphere, cloud_material.density, cloud_material);

            Box::new(volume) as Box<dyn Hittable>
        },
        0.5, // spacing factor
    );

    for cloud in clouds.generate() {
        scene.add_boxed_object(cloud);
    }
}

fn add_tree(scene: &mut Scene, x: f32, y: f32, z: f32) {
    scene.add_object(Cylinder::new(
        Point3::new(x, y , z),
        0.2, 1.2,
        Material { 
            texture: Texture::SolidColor(Color::new(0.8, 0.6, 0.4)),
            diffuse: 1.0, 
            reflectivity: 0.0,
            transparency: 0.0,
            ior: 0.0,
            emission: None,
            specular: 0.0,
            shininess: 0.0,
            is_volume: false,
            density: 0.0,
        })
    );

    scene.add_object(Sphere::new(
        Point3::new(0.0 + x, 1.92 + y, 0.0 + z),
        0.75,
        Material { 
            texture: Texture::SolidColor(Color::GREEN),
            diffuse: 0.0, 
            reflectivity: 0.0,
            transparency: 0.6,
            ior: 1.333,
            emission: None,
            specular: 0.0,
            shininess: 128.0,
            is_volume: false,
            density: 0.0,
        })
    );

    scene.add_object(Sphere::new(
        Point3::new(0.6 + x, 1.2 + y, 0.0 + z),
        0.4,
        Material { 
            texture: Texture::SolidColor(Color::GREEN),
            diffuse: 0.0, 
            reflectivity: 0.0,
            transparency: 0.6,
            ior: 1.333,
            emission: None,
            specular: 0.0,
            shininess: 128.0,
            is_volume: false,
            density: 0.0,
        })
    );

    scene.add_object(Sphere::new(
        Point3::new(0.0 + x, 1.3 + y, 0.6 + z),
        0.3,
        Material { 
            texture: Texture::SolidColor(Color::GREEN),
            diffuse: 0.0, 
            reflectivity: 0.0,
            transparency: 0.6,
            ior: 1.333,
            emission: None,
            specular: 0.0,
            shininess: 128.0,
            is_volume: false,
            density: 0.0,
        })
    );
}

fn add_flower(scene: &mut Scene, x: f32, y: f32, z: f32, size: f32) {
    scene.add_object(Sphere::new(
        Point3::new(x, y + size * 0.4, z),
        size * 0.5,
        Material { 
            texture: Texture::SolidColor(Color::YELLOW),
            diffuse: 1.0, 
            reflectivity: 0.0,
            transparency: 0.0,
            ior: 0.0,
            emission: None,
            specular: 0.0,
            shininess: 0.0,
            is_volume: false,
            density: 0.0,
        })
    );
    
    let offset = size * 0.6;
    let size = size * 0.7;
    scene.add_object(Cylinder::new(
        Point3::new(x + offset, y , z),
        size, 0.02,
        Material { 
            texture: Texture::SolidColor(Color::RED),
            diffuse: 1.0, 
            reflectivity: 0.0,
            transparency: 0.0,
            ior: 0.0,
            emission: None,
            specular: 0.0,
            shininess: 0.0,
            is_volume: false,
            density: 0.0,
        })
    );
    
    scene.add_object(Cylinder::new(
        Point3::new(x - offset, y, z),
        size, 0.02,
        Material { 
            texture: Texture::SolidColor(Color::RED),
            diffuse: 1.0, 
            reflectivity: 0.0,
            transparency: 0.0,
            ior: 0.0,
            emission: None,
            specular: 0.0,
            shininess: 0.0,
            is_volume: false,
            density: 0.0,
        })
    );
    
    scene.add_object(Cylinder::new(
        Point3::new(x, y, z + offset),
        size, 0.02,
        Material { 
            texture: Texture::SolidColor(Color::RED),
            diffuse: 1.0, 
            reflectivity: 0.0,
            transparency: 0.0,
            ior: 0.0,
            emission: None,
            specular: 0.0,
            shininess: 0.0,
            is_volume: false,
            density: 0.0,
        })
    );
    
    scene.add_object(Cylinder::new(
        Point3::new(x, y, z - offset),
        size, 0.03,
        Material { 
            texture: Texture::SolidColor(Color::RED),
            diffuse: 1.0, 
            reflectivity: 0.0,
            transparency: 0.0,
            ior: 0.0,
            emission: None,
            specular: 0.0,
            shininess: 0.0,
            is_volume: false,
            density: 0.0,
        })
    );

}

fn random_material() -> Material {
    let mut rng = rand::rng();

    let color = Color::new(random_double(), random_double(), random_double());
    let texture = Texture::SolidColor(color);

    match rng.random_range(0..3) {
        0 => Material {
            texture,
            diffuse: 0.5,
            reflectivity: 0.0,
            transparency: 0.0,
            ior: 0.0,
            emission: None,
            specular: 0.0,
            shininess: 0.0,
            is_volume: false,
            density: 0.0,
        },
        1 => Material {
            texture,
            diffuse: rng.random_range(0.01..0.2),
            reflectivity: 0.9,
            transparency: 0.0,
            ior: 0.0,
            emission: None,
            specular: 6.0,
            shininess: 128.0,
            is_volume: false,
            density: 0.0,
        },
        2 => Material {
            texture,
            diffuse: 0.0,
            reflectivity: 0.0,
            transparency: rng.random_range(0.5..0.95),
            ior: rng.random_range(1.3..1.7),
            emission: None,
            specular: 3.0,
            shininess: 64.0,
            is_volume: false,
            density: 0.0,
        },
        _ => unreachable!(),
    }
}